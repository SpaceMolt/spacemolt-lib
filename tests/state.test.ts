...
test('seed populates sections and reports them changed', () => {
  ...
  expect(changed.sort()).toEqual(['cargo', 'location', 'player', 'queue', 'ship', 'skills'].sort() as StateSection[]);
});
...
test('applyDelta replaces present sections and leaves absent ones untouched', () => {
  ...
  expect(changed.sort()).toEqual(['cargo', 'queue', 'ship'].sort() as StateSection[]);
...
async function seededAccount(): Promise<{ account: Account; socket: MockSocket }> {
  ...
  socket.onClientSend = (frame, s) => {
    if (frame.action === 'login') {
      ...
      s.serverSend({ type: 'result', request_id: frame.request_id, payload: { result: 'ok', structuredContent: SNAPSHOT } });
    }
  };
